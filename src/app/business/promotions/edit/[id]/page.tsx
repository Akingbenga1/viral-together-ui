...
export default function EditPromotionPage() {
  ...
  const [errors, setErrors] = useState<Partial<UpdatePromotionData & { social_media_platform_id?: string }>>({});
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<UpdatePromotionData> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const promotionData = {
        business_id: user?.business_id || 1, // Add required business_id field
        promotion_name: formData.promotion_name?.trim() || '',
        promotion_item: formData.promotion_item?.trim() || '',
        description: formData.description?.trim() || null,
        start_date: new Date(formData.start_date).toISOString(),
        end_date: new Date(formData.end_date).toISOString(),
        discount: formData.discount && formData.discount > 0 ? parseFloat(formData.discount.toString()) : null,
        budget: formData.budget && formData.budget > 0 ? parseFloat(formData.budget.toString()) : null,
